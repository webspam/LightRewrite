...
function LRDebug_HasEdits(params: CLightRewriteSourceParams): bool {
    ...
    return params.hasBrightness ||
        params.hasRadius ||
        params.hasAttenuation ||
        params.hasShadowFadeDistance ||
        params.hasShadowFadeRange ||
...
function LRDebug_BuildExportLine(
    ...
    if (params.hasShadowFadeDistance) {
        line += " shadowFadeDistance=" + FloatToString(params.shadowFadeDistance);
    }
    if (params.hasShadowFadeRange) {
        line += " shadowFadeRange=" + FloatToString(params.shadowFadeRange);
    }
    if (params.hasShadowBlendFactor) {
        line += " shadowBlendFactor=" + FloatToString(params.shadowBlendFactor);
    }
